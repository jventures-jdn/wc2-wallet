...
export default function useWalletConnectEventsManager(initialized: boolean) {
  ...
  const onSessionRequest = useCallback(
    async (tempRequestEvent: SignClientTypes.EventArguments['session_request']) => {
      const { topic, params } = tempRequestEvent
      const { request } = params
      const requestSession = signClient.session.get(topic)

      console.log(requestSession)

      let requestEvent = tempRequestEvent
      requestEvent = {
        ...requestEvent,
        params: {
          ...requestEvent.params,
            ...
            params: [
              {
                ...
                gasLimit: requestEvent.params.request.params[0].gas
              }
            ]
          }
        }
      ...
      delete requestEvent.params.request.params[0].gas
